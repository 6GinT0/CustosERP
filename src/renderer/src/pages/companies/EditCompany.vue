...
const { company, loading, mapReady, address, setFieldValue, handleUpdate } = useCompanyActions()
...
watchEffect(() => {
  if (company.value) {
    ...
    marketLatLng.value = [company.value.latitude!, company.value.longitude!]
  }
})
</script>
    ...
    <v-card flat class="border pa-4">
      <form @submit.prevent="handleUpdate">
        <v-row class="pa-4">
          ...
          <v-col cols="12">
            <v-btn type="submit" color="primary" flat>Editar</v-btn>
          </v-col>
        </v-row>
      </form>
    </v-card>
